...
if ($vcDHCP->exists('.')) {
    ...
    if ($vcDHCP->exists('enable-hostfile-update')) {
        ...
        $genout_initial .= "\tset ClientIp = binary-to-ascii(10, 8, \".\", leased-address);\n";
        $genout_initial .= "\tset ClientMac = binary-to-ascii(16, 8, \":\", substring(hardware, 1, 6));\n";
        $genout_initial .= "\tset ClientDomain = pick-first-value(config-option domain-name, \"..YYZ!\");\n";
        $genout_initial .= "\texecute(\"/opt/vyatta/sbin/on-dhcp-event.sh\", \"commit\", ClientName, ClientIp, ClientMac, ClientDomain);\n";
        $genout_initial .= "}\n\n";
        ...
        $genout_initial .= "\tset ClientMac = binary-to-ascii(16, 8, \":\", substring(hardware, 1, 6));\n";
        $genout_initial .= "\tset ClientDomain = pick-first-value(config-option domain-name, \"..YYZ!\");\n";
        $genout_initial .= "\texecute(\"/opt/vyatta/sbin/on-dhcp-event.sh\", \"release\", ClientName, ClientIp, ClientMac, ClientDomain);\n";
        $genout_initial .= "}\n\n";
...
EOM
						...
						my $failover_local_port = $vcDHCP->returnValue("$name subnet $subnet failover local-address $failover_local_address port");
                        my $failover_peer_address = $vcDHCP->returnValue("$name subnet $subnet failover peer-address");
						my $failover_peer_port = $vcDHCP->returnValue("$name subnet $subnet failover peer-address $failover_peer_address port");
