...
$genout_initial .=  "# using 'changes only' directory:   $vcDHCP->{_changes_only_dir_base}\n";
$genout_initial .=  "# using 'new config' directory:     $vcDHCP->{_new_config_dir_base}\n\n";
			...
			if (@subnets == 0) {
				...
				foreach my $subnet (@subnets) {
					...
					if (defined($naipNetwork)) {
						...
						if ($client_prefix_length ne '') {
					        ...
					        $genout .= "\t\tpool {\n";	#for  pool specific parameters, options for the subnet should not be inside this
	                                        ...
	                                        if ($failover_local_address ne '' && $failover_peer_address ne '' && $failover_name ne '' && $failover_status ne '') {
	                                            ...
	                                            $genout .= "\t\t\tfailover peer \"$failover_name\";\n";
	                                           ...
	                                           # do nothing, basically failover has not been configured
	                                                ...
	                                                print stderr "Please set one or more of the missing DHCP failover parameters:\n";
	                                                print stderr "local-address | peer-address | name | status\n";
	                                                $error = 1;
	                                        }
						...
						if (@ranges == 0) {
                                		   print stderr "DHCP server configuration error.  No DHCP start-stop range set for subnet $subnet\n";
